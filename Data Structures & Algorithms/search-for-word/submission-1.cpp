...
public:
    ...
    bool processChar(vector<vector<char>>& board, const string& word, int word_pos, int curr_i, int curr_j) {
        ...
        if (isValidCoords(curr_i - 1, curr_j, board)) {
            if (processChar(board, word, word_pos + 1, curr_i - 1, curr_j)) {
                return true;
            }
        }

        if (isValidCoords(curr_i, curr_j + 1, board)) {
            if (processChar(board, word, word_pos + 1, curr_i, curr_j + 1)) {
                return true;
            }
        }
