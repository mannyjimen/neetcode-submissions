...
public:
    ...
    void evict() {
        ...
        back->next = evicted->next;
        evicted->next->prev = back;
...
current = x
current_capacity++
current = new node(1, 10). Ox1-[1, 10]
nodes[key] = current
insertNode(0x1)
    front->prev(back)->next = 0x1    back->0x1
    current(0x1)->prev = front->prev(back)   back<->0x1
    front->prev = current(0x1)    back<->0x1<-front
    current(0x1)->next = front    back<->0x1<->front
    done.
LIST LOOKS LIKE:
back<->0x1<->front
---
get(1)
moveToFront(nodes[1]=0x1)
    current(0x1)->prev(back)->next = current(0x1)->next(front)
    current(0x1)->next(front)->prev = current(0x1)->prev(back)
    back<->front    0x1

    front->prev(back)->next = current(0x1)
    current(0x1)->prev = front->prev(back)
    front.prev->back<->0x1->

    front->prev=current(0x1)
    current->next = front
    back<->0x1<->front
